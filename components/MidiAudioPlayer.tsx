...
// Icons (reusing from AudioPlayer)
...
interface MidiAudioPlayerProps {
  midiBlob?: Blob;
  filename?: string;
  credit?: string;
}

const MidiAudioPlayer: React.FC<MidiAudioPlayerProps> = ({ midiBlob, filename, credit }) => {
  ...
  // Create audio URL from MIDI blob
  useEffect(() => {
    if (midiBlob) {
      setIsLoading(true);
      setError(null);
  ...
  const togglePlayPause = () => {
    ...
    if (!prevValue) {
      audioRef.current.play().catch((err) => {
        ...
        setError('Failed to play MIDI file. Your browser may not support MIDI playback.');
        setIsPlaying(false);
      });
  ...
  const handleError = () => {
    setError('Failed to load MIDI file. Your browser may not support MIDI playback.');
    setIsLoading(false);
  };
    ...
    return (
      <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg shadow-md border border-slate-200 dark:border-slate-600">
        <div className="text-center text-slate-500 dark:text-slate-400 py-8">
          <p>No MIDI file available</p>
          <p className="text-sm mt-1">Convert a MusicXML file to generate MIDI audio</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg shadow-md border border-slate-200 dark:border-slate-600">
        <div className="text-center text-red-600 dark:text-red-400 py-8">
          ...
          <p className="text-sm mt-1 text-slate-500 dark:text-slate-400">
            Note: MIDI playback requires browser support and may not work in all browsers
          </p>
        </div>
      </div>
    );
    ...
    <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg shadow-md border border-slate-200 dark:border-slate-600">
      {audioUrl && (
        ...
        >
          ...
          <source src={audioUrl} type="audio/mid" />
          Your browser does not support MIDI audio playback.
        </audio>
      )}
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        <button 
          ...
          disabled={!audioUrl}
          className="text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300 disabled:text-slate-400 focus:outline-none flex-shrink-0" 
        ...
        <div className="flex-1 flex items-center space-x-2 sm:space-x-3 min-w-0">
          <span className="text-sm font-mono text-slate-600 dark:text-slate-300 w-12 text-center">
          ...
          <input 
            ...
            disabled={!audioUrl}
            className="w-full h-1.5 bg-slate-300 dark:bg-slate-500 rounded-full appearance-none cursor-pointer disabled:cursor-not-allowed" 
        ...
        <div className="hidden sm:flex items-center space-x-2">
          <button 
            onClick={toggleMute} 
            disabled={!audioUrl}
            className="text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-slate-100 disabled:text-slate-400" 
          ...
          <input 
            ...
            disabled={!audioUrl}
            className="w-20 h-1.5 bg-slate-300 dark:bg-slate-500 rounded-full appearance-none cursor-pointer disabled:cursor-not-allowed" 
            aria-label="Volume"
          />
        </div>
      </div>
      ...
      {credit && (
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 text-center sm:text-right">
          {credit}
        </p>
      )}
    </div>

...
export const MatchmakingView: React.FC = () => {
    ...
    const handleBotMatch = async () => {
        setStatus('joining');
        try {
            const { success, match_id, error } = await import('../../services/rpgService').then(m => m.startBotMatch());
            if (success && match_id) {
                setStatus('found');
                setTimeout(() => {
                    navigate(`/rpg/battle/${match_id}`);
                }, 1500);
            } else {
                setError(error || 'Failed to start bot match');
                setStatus('searching');
            }
        } catch (e) {

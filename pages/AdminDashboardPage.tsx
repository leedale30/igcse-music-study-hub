...
const AdminDashboardPage: React.FC = () => {
  ...
  const loadStudentData = async () => {
    ...
    try {
      ...
      // 2. Fetch all progress data for these students
      ...
      const studentSummaries: StudentSummary[] = studentsData.map(profile => {
        ...
        const quizResults: QuizResult[] = studentProgressItems.map(item => ({
          quizId: item.quiz_id,
          quizTitle: item.answers?.quizTitle || `Quiz ${item.quiz_id}`,
        ...
        let lastActivity = new Date(profile.created_at); // Default to creation time
        if (quizResults.length > 0) {
          const lastQuizDate = new Date(Math.max(...quizResults.map(q => q.completedAt.getTime())));
          if (lastQuizDate > lastActivity) lastActivity = lastQuizDate;
        }

        // Create StudentSummary
        const user: User = {
          id: profile.id,
          email: profile.email || '', // Profiles might not have email directly if protected, check schema
          ...
          role: 'student',
          group: profile.group_name || profile.grade || 'Unassigned',
          profileCompleted: true, // Assuming profile exists
          createdAt: new Date(profile.created_at),
          lastLoginAt: new Date(profile.updated_at) // Approximate
        };
        ...
        const progress: StudentProgress = {
          ...
          pageProgress: [],
          badges: [], // Fetch from achievements table if implemented
        ...
        return {
          ...
          recentQuizzes: quizResults.sort((a, b) => b.completedAt.getTime() - a.completedAt.getTime()).slice(0, 5),
          badges: [],
